...
# Get tag info (filter out lightweight tags)
try{
	$tagsInfo = ($result | ? {$_.object.type -eq "tag"} | % { "$($githubRepoApiUri)git/tags/$($_.object.sha)" ;} | % { Invoke-RestMethod -Method 'GET' -Uri $($_)})
}
catch [System.Net.WebException] 
{	
	# Error getting tags
